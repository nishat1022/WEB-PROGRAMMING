...
// Helper: send JSON and close connection
...
if ($isUpload && $method === 'POST') {
    if (!isset($_FILES['image'])) {
        sendJSON(["error" => "No 'image' field in the request. Use multipart/form-data."], 400);
        exit;
    }
    ...
    // Generate unique filename with collision safety (max 3 retries)
    $filename = null;
    $destination = null;
    $maxRetries = 3;
    for ($i = 0; $i < $maxRetries; $i++) {
        $candidate = uniqid('img_', true) . '.' . $ext;
        $candidatePath = $uploadDir . $candidate;
    ...
    if ($filename === null) {
        sendJSON(["error" => "Could not generate a unique filename. Please try again."], 500);
        exit;
    }
...
switch ($method) {
    ...
    case 'GET':
        ...
        if (!$stmt) {
            sendJSON(["error" => "Query preparation failed"], 500);
            break;
        }
        if ($params) {
    ...
    case 'POST':
        ...
        // Sanitise values — empty string for a DATE column must become NULL
        ...
        if ($stmt->error) {
            ...
            sendJSON(["error" => "An internal error occurred while creating the news item"], 500);
            break;
        }
        ...
        if (!$row) {
            sendJSON(["error" => "Document not found after create"], 500);
            break;
        }
    ...
    case 'PUT':
        ...
        // Return the full updated document regardless of affected_rows
        $fetch = $conn->prepare("SELECT * FROM documents WHERE id = ?");
        if (!$fetch) {
            sendJSON(["error" => "Query preparation failed"], 500);
            break;
        ...
        if (!$row) {
            sendJSON(["error" => "Document not found after update"], 500);
            break;
        }
    ...
    case 'DELETE':
        ...
        // Fetch image_url before deleting so we can clean up the file
        $fetchImg = $conn->prepare("SELECT image_url FROM documents WHERE id = ?");
        if ($fetchImg) {
            $fetchImg->bind_param("i", $id);
            $fetchImg->execute();
            $imgResult = $fetchImg->get_result();
            if ($imgRow = $imgResult->fetch_assoc()) {
                $oldImageUrl = $imgRow['image_url'] ?? '';
            }
            $fetchImg->close();
        }
        ...
        if ($stmt->affected_rows === 0) {
            ...
            // Clean up orphaned uploaded image file
            if (!empty($oldImageUrl) && strpos($oldImageUrl, 'uploads/') === 0) {
